...
export const getPresets = async (req, res) => {
  try {
    const presets = await presetDimensionsModel.find({
      $or: [{ owner: req.userId }, { owner: '6409f695c0d9247e878e36cf' }]
    });
...
export const createPreset = async (req, res) => {
  try {
    ...
    console.log(
      chalk.red.bold(`[${new Date().toLocaleString()}] POST /dimensions/ ${error.message}`)
    );
